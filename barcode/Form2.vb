...
Public Class Form2
    ...
    Private Sub getuserdetails()
        ...
        lblsitet2.Text = user_site
        lblusert2.Text = empnum
        lblshiftt2.Text = user_shift
        lblnamet2.Text = empname
        txtoper_num.Text = empnum
        txtoper_name.Text = empname

        txt_empnum_goodtags.Text = empnum
        txt_site_goodtags.Text = user_site
        txt_shift_goodtags.Text = user_shift

        txt_empnum_nctags.Text = empnum
        txt_shift_nctags.Text = user_shift
        txt_site_nctags.Text = user_site
        txtoper_numt3.Text = empnum
        txtoper_namet3.Text = empname

        txtempname_subcon.Text = empname
        txtempnum_subcon.Text = empnum
        txtempnum_subcon_hdr.Text = empnum
        txtsite_subcon.Text = user_site
        txtshift_subcon.Text = user_shift

        lblsite_reports.Text = user_site
        lblempnum_reports.Text = empnum
        lblshift_reports.Text = user_shift
        lblempname_report.Text = empname
        ...
        'cmd1.Parameters.AddWithValue("@site", Form1.cmb_site.Text)
        ...
        Try
            ...
            Using con As New SqlConnection("Data Source=ERP-SVR;Initial Catalog=Pallet_Tagging;User ID=sa;Password=pi_dc_2011")
                ...
                Using cmd As New SqlCommand("Select MAX(Tag_num) FROM NCFPtag_Line WHERE Site=@site AND Job=@job AND Suffix=@suffix AND Oper_num=@opernum", con)
                    cmd.Parameters.AddWithValue("@site", txt_site_nctags.Text)
        ...
        Try
            ...
            Using con As New SqlConnection("Data Source=ERP-SVR;Initial Catalog=Pallet_Tagging;User ID=sa;Password=pi_dc_2011")
                ...
                Using cmd As New SqlCommand("SELECT MAX(Tag_num) FROM Ptag_Line WHERE Site=@site AND Job=@job AND Suffix=@suffix AND Oper_num=@opernum", con)
                    cmd.Parameters.AddWithValue("@site", txt_site_goodtags.Text)
    ...
    Private Sub btngenerate_Click(sender As Object, e As EventArgs) Handles btngenerate.Click
        ...
        Try
            ...
            cmd.Parameters.AddWithValue("@shift", txt_shift_goodtags.Text)
            ...
            Try
                ...
                For Each row As DataGridViewRow In DataGridView3.Rows
                    ...
                    If Not row.IsNewRow Then
                        ...
                        cmd.Parameters("@site").Value = txt_site_nctags.Text
    ...
    Private Sub btngeneratet3_Click(sender As Object, e As EventArgs) Handles btngeneratet3.Click
        ...
        Try
            ...
            cmd3.Parameters.AddWithValue("@site", txt_site_nctags.Text)
            ...
            cmd3.Parameters.AddWithValue("@shift", txt_shift_nctags.Text)
            ...
            Try
                ...
                For Each row As DataGridViewRow In DataGridView1.Rows
                    ...
                    If Not row.IsNewRow Then
                        ...
                        cmd.Parameters("@tagnum").Value = row.Cells("Tag #").Value
                        cmd.Parameters("@site").Value = txt_site_goodtags.Text
    ...
    Private Sub Button24_Click(sender As Object, e As EventArgs) Handles Button24.Click
        ...
        Try
            con1.Open()
            Dim cmd_updatejobqty As New SqlCommand("select j.job,j.suffix, j.item,j.description ,itm.Uf_itemdesc_ext, j.qty_released as job_qty, itm.u_m
                        from job j inner join item itm on itm.item=j.item
                        where j.job=@jonumber and j.suffix=@josuffix", con1)
            cmd_updatejobqty.Parameters.AddWithValue("@jonumber", txtjoborder.Text)
            cmd_updatejobqty.Parameters.AddWithValue("@josuffix", txtjobsuffix.Text)

            Dim read_cmdupdatejobqty As SqlDataReader = cmd_updatejobqty.ExecuteReader
            If read_cmdupdatejobqty.HasRows Then
                While read_cmdupdatejobqty.Read
                    txtjobqty.Text = read_cmdupdatejobqty("job_qty").ToString
                End While
                read_cmdupdatejobqty.Close()
                con1.Close()
                Try
                    con.Open()
                    Dim cmd_ptaghdrjobqtyupdate As New SqlCommand("Update Ptag_hdr Set Job_qty = @jobqty WHERE Job = @job AND Suffix = @suffix", con)
                    cmd_ptaghdrjobqtyupdate.Parameters.AddWithValue("@job", txtjoborder.Text)
                    cmd_ptaghdrjobqtyupdate.Parameters.AddWithValue("@suffix", txtjobsuffix.Text)
                    cmd_ptaghdrjobqtyupdate.Parameters.AddWithValue("@jobqty", txtjobqty.Text)

                    cmd_ptaghdrjobqtyupdate.ExecuteNonQuery()
                    MsgBox("Job Qty Updated Successfully!")
    ...
    Private Sub BTN_MonitoringReport_Click(sender As Object, e As EventArgs) Handles BTN_MonitoringReport.Click
        FRM_Monitoring_Checklist_Report.Show()
        Me.Close()
    End Sub
End Class